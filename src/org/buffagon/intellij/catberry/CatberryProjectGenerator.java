...
public class CatberryProjectGenerator extends WebProjectTemplate<Object> {
    ...
    @Override
    public void generateProject(@NotNull Project project, @NotNull final VirtualFile baseDir, @NotNull Object data, @NotNull final Module module) {
        ApplicationManager.getApplication().runWriteAction(
            new Runnable() {
                public void run() {
                    ...
                    try {
                        Process process = new ProcessBuilder().directory(new File(baseDir.getPath())).command("touch", "p.txt").start();
                        // TODO: 07.03.16 cli generator
                        process.waitFor();
                    } catch (IOException e) {
                        e.printStackTrace();
